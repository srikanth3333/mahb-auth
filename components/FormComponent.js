...
//   let dispatch = useDispatch();
  ...
  const handleSubmit = (values) => {
    ...
    let formName = `Form ${Math.floor(Math.random() * 10)}`
    let mulForms =  localStorage.getItem('forms');
    let combinedObj = !mulForms ? [{title:newTitle,data:finalObjArr}]  : [...JSON.parse(mulForms), {title:newTitle,data:finalObjArr}]
    localStorage.setItem('forms',JSON.stringify(combinedObj))
    alert("Form saved got to forms")
    location.reload()
  }
                            ...
                            <>
                                <div className={!colFix ? 'col-lg-3' : colFix}>
                                    <Input 
                                        ...
                                        onChange={(e) => handleLabelEdit(e,item.value)}
                                        className='editable-input'                                        
                                    />
                                    <Form.Item 
                                        name={item.label}
                            ...
                            <>
                                <div className={!colFix ? 'col-lg-3' : colFix}>
                                    ...
                                    <Form.Item name={item.label}
                                            rules={[{ required: item.required, message: `Please input your ${item.label}` }]}
                                        >
                                                <Radio.Group disabled={true} onChange={() => {
